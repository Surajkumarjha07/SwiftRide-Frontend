...
export default function ChatModal() {
    ...
    return (
        <aside className={`${isDarkMode ? "bg-gray-800 shadow-none" : "bg-white shadow-sm shadow-gray-400"} ${isChatOpen ? 'opacity-100 h-[28rem] z-50' : 'opacity-0 h-0 -z-10'} w-80 flex flex-col rounded-2xl overflow-hidden transition-all duration-500`}>
            <div className='flex justify-between items-center h-[12%] px-4'>
                <p className={`${isDarkMode ? "text-white" : "text-gray-700"} font-medium text-xl`}> Messages </p>
                <button onClick={closeChatModal}>
                    <Plus className={`rotate-45 cursor-pointer ${isDarkMode ? "text-white" : "text-gray-800"}`} />
                </button>
            </div>
